...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#svaeRemark").click(function() {
			
			
		 	$.ajax({
				
					url:"workbench/activityRemark/svaeRemark.do",
					data:{
					...
					success:function (data){
						//返回一个true false  一个对象ar
						if(data.success){
							var html = "";
							html +='<div id="'+data.ar.id+'" class="remarkDiv" style="height: 60px;">';                                                                                         
							html +='<img title="'+data.ar.id+'" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';                                                       
							html +='<div style="position: relative; top: -40px; left: 40px;" >';                                                                                    
							html +='<h5>'+data.ar.noteContent+'</h5>';                                                                                                                                
							html +='<font color="gray">市场活动</font> <font color="gray">-</font> <b>'+data.ar.noteContent+'</b> <small style="color: gray;"> '+data.ar.createTime+' 由'+data.ar.createBy+'</small>'; 
	 			...
	 			$.ajax({
					...
					success:function(data){
						if(data.success){
							...
							//返回一个true或false 
							//返回一个remark,editData, editBy,noteContent
	...
	function showRemark() {
		$.ajax({
			url:"workbench/activityRemark/findRemark.do",
			data:{
				"aid":"${a.id}"
			},
			type:"get",
			...
			success:function(data){
				/* 返回一个activityRemark
				需要name 创建人：owner、创建时间：startDate；
				*/
		...
		<div id="remarkDiv" style="background-color: #E6E6E6; width: 870px; height: 90px;">
			<form role="form" style="position: relative;top: 10px; left: 10px;">
				...
				<p id="cancelAndSaveBtn" style="position: relative;left: 737px; top: 10px; display: none;">
					...
					<button type="button" class="btn btn-primary" id="svaeRemark">保存</button>
				</p>
			</form>
		</div>
	</div>
